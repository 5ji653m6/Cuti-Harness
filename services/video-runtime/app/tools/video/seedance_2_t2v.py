"""
ByteDance Seedance 2.0 Text-to-Video（WaveSpeed API）

Pure text-to-video. Supports optional reference_images / reference_videos / reference_audios as style/character/audio references.
...
class Seedance2T2vVideoInput(BaseModel):
...
@tool(ToolName.SEEDANCE_2_T2V, args_schema=Seedance2T2vVideoInput)
async def generate_video_with_wavespeed_seedance_2_t2v(
    t2v_prompt: str,
    ...
    """Generate video with ByteDance Seedance 2.0 Text-to-Video (WaveSpeed). 480p/720p/1080p API; native audio (generate_audio=True); optional reference images/videos/audios."""
    try:
        ...
        async def _make_request(api_key: str):
            ...
            svc.api_key = api_key
            return await svc.generate_seedance_2_t2v(
                prompt=t2v_prompt,
                duration=duration,
        ...
        result = await router.route_tool_request(
            ...
            tool_type=ToolType.SEEDANCE_2_T2V,
            request_func=_make_request
        )
        if result.success:
            result = result.model_copy(update={
                "model": ToolType.SEEDANCE_2_T2V.value,
                "aspect_ratio": aspect_ratio,
                "resolution": user_res_enum.value,
            })
            cost = ToolService.calculate_cost(
                cost_type=ToolType.SEEDANCE_2_T2V,
                duration=duration,
                resolution=user_res_enum,
            )
            if cost > 0:
                ToolService.log_cost(cost)
                logger.info(f"💰 Seedance 2.0 T2V 成本: ${cost:.6f} ({duration}s, {resolution_api})")
                cb = ToolService.get_credit_callback(getattr(runtime, "config", None) if runtime else None)
                if cb:
                    cb.add_tool_cost(cost, tool_name=ToolName.SEEDANCE_2_T2V.value, tool_type=ToolType.SEEDANCE_2_T2V)
            result = result.model_copy(update={"billing_cost": cost})
            return result
        return result
    except Exception as e:
        logger.error(f"❌ Seedance 2.0 T2V 异常: {str(e)}")
