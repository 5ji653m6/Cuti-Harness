"""
ByteDance Seedance 2.0 Fast Text-to-Video Turbo（WaveSpeed API）

The fastest, cheapest HD turbo pure text-to-video: 720p / 1080p only (480p uses 720p).
Supports optional reference_images / reference_videos / reference_audios as style/character/audio references.
Resolution / ratio come from runtime.context; generate_audio defaults to True (the API default).
"""
import logging
import random
...
class Seedance2FastT2vTurboVideoInput(BaseModel):
    """Input schema for Seedance 2.0 Fast Text-to-Video Turbo."""
...
@tool(ToolName.SEEDANCE_2_FAST_T2V_TURBO, args_schema=Seedance2FastT2vTurboVideoInput)
async def generate_video_with_wavespeed_seedance_2_fast_t2v_turbo(
    t2v_prompt: str,
    ...
    """Generate video with ByteDance Seedance 2.0 Fast Text-to-Video Turbo (WaveSpeed). 720p/1080p API (480p via downscale); native audio (generate_audio=True); optional reference images/videos/audios; fastest and cheapest Seedance HD option."""
    try:
        ...
        async def _make_request(api_key: str):
            ...
            svc.api_key = api_key
            return await svc.generate_seedance_2_fast_t2v_turbo(
                prompt=t2v_prompt,
                duration=duration,
        ...
        result = await router.route_tool_request(
            ...
            tool_type=ToolType.SEEDANCE_2_FAST_T2V_TURBO,
            request_func=_make_request
        )
        if result.success:
            result = result.model_copy(update={
                "model": ToolType.SEEDANCE_2_FAST_T2V_TURBO.value,
                "aspect_ratio": aspect_ratio,
                "resolution": user_res_enum.value,
            })
            cost = ToolService.calculate_cost(
                cost_type=ToolType.SEEDANCE_2_FAST_T2V_TURBO,
                duration=duration,
                resolution=api_res_enum,
            )
            if cost > 0:
                ToolService.log_cost(cost)
                logger.info(f"💰 Seedance 2.0 Fast T2V Turbo 成本: ${cost:.6f} ({duration}s, {resolution_api})")
                cb = ToolService.get_credit_callback(getattr(runtime, "config", None) if runtime else None)
                if cb:
                    cb.add_tool_cost(cost, tool_name=ToolName.SEEDANCE_2_FAST_T2V_TURBO.value, tool_type=ToolType.SEEDANCE_2_FAST_T2V_TURBO)
            result = result.model_copy(update={"billing_cost": cost})
            return result
        return result
    except Exception as e:
        logger.error(f"❌ Seedance 2.0 Fast T2V Turbo 异常: {str(e)}")
