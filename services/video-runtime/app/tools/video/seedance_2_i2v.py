"""
ByteDance Seedance 2.0 Image-to-Video (WaveSpeed API, not Fast)
"""
import logging
import random
...
class Seedance2I2vVideoInput(BaseModel):
...
@tool(ToolName.SEEDANCE_2_I2V, args_schema=Seedance2I2vVideoInput)
async def generate_video_with_wavespeed_seedance_2_i2v(
    i2v_prompt: str,
    start_image_url: str,
    ...
    """Generate video with ByteDance Seedance 2.0 Image-to-Video (WaveSpeed). 480p/720p/1080p API; optional last frame; generate_audio from context (Short Drama in-clip dialogue)."""
    try:
        ...
        async def _make_request(api_key: str):
            ...
            svc.api_key = api_key
            return await svc.generate_seedance_2_i2v(
                image=final_start,
                prompt=i2v_prompt,
                duration=duration,
        ...
        result = await router.route_tool_request(
            ...
            tool_type=ToolType.SEEDANCE_2_I2V,
            request_func=_make_request
        )
        if result.success:
            result = result.model_copy(update={
                "model": ToolType.SEEDANCE_2_I2V.value,
                "aspect_ratio": aspect_ratio,
                "resolution": user_res_enum.value,
            })
            cost = ToolService.calculate_cost(
                cost_type=ToolType.SEEDANCE_2_I2V,
                duration=duration,
                resolution=user_res_enum,
            )
            if cost > 0:
                ToolService.log_cost(cost)
                logger.info(f"💰 Seedance 2.0 I2V 成本: ${cost:.6f} ({duration}s, {resolution_api})")
                cb = ToolService.get_credit_callback(getattr(runtime, "config", None) if runtime else None)
                if cb:
                    cb.add_tool_cost(cost, tool_name=ToolName.SEEDANCE_2_I2V.value, tool_type=ToolType.SEEDANCE_2_I2V)
            result = result.model_copy(update={"billing_cost": cost})
            return result
        return result
    except Exception as e:
        logger.error(f"❌ Seedance 2.0 I2V 异常: {str(e)}")
